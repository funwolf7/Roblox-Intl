...
local function exp(val, engineering)
	...
	if #intg == 0 then
		...
		return negt
			.. (frac:sub(fsize + 1, fsize + size) .. ("0"):rep(math.max((fsize + size) - #frac, 0)))
			.. ("." .. frac:sub(fsize + size + 1)):gsub("%.0*$", "")
			.. ("E-" .. fsize + size):gsub("-0+$", "0")
	end
	local size = engineering and (((#intg - 1) % 3) + 1) or 1
	return negt
		.. (("0"):rep(math.max(size - #intg, 0)) .. intg:sub(1, size))
		.. ("." .. intg:sub(size + 1) .. frac):gsub("%.0*$", "")
		.. "E"
		.. (#intg - size)
end
...
local function format_pattern(self, start, value, pattern, negt, compact, source)
	...
	for i, v in ipairs(pattern) do
		...
		elseif v == 1 then
			ret = checker.addpart(
				ret,
				negt and "minusSign" or "plusSign",
				negt and self.symbols.minusSign or self.symbols.plusSign,
				source
			)
		elseif v == 2 then
		...
		elseif v == 6 then
			if self.currencyDisplay ~= "hidden" then
				local symbol = (
					self.currencyData
					and (
						(self.currencyDisplay == "narrowSymbol" and self.currencyData["symbol-alt-narrow"])
				...
				if (pattern[i + 1] == 0) and symbol:match("%a$") then
					symbol = symbol
						.. (self.currencySpacing and self.currencySpacing.afterCurrency.insertBetween or " ")
				end
				if (pattern[i - 1] == 0) and symbol:match("^%a") then
					symbol = (self.currencySpacing and self.currencySpacing.beforeCurrency.insertBetween or " ")
						.. symbol
				end
...
function format(self, parts, value0, value1)
	...
	for i = 0, range and 1 or 0 do
		...
		if post:match("^[%d.]*$") and select(2, post:gsub("%.", "")) < 2 then
			...
			if self.notation == "compact" then
				...
				if not (minfrac or maxfrac) then
					maxfrac = math.max(
						2
							- ((compact_val:find("%.") or (#compact_val + 1)) - 1)
							+ #compact_val:gsub("%.", ""):match("^0*"),
						0
					)
				end
				...
				if self.isSignificant then
					formatted_compact_val = checker.raw_format_sig(
						negt,
						compact_val,
						self.minimumSignificantDigits,
						self.maximumSignificantDigits,
					...
					formatted_compact_val = checker.raw_format(
						negt,
						compact_val,
						self.minimumIntegerDigits,
						self.maximumIntegerDigits,
				...
				if #formatted_compact_val:gsub("%.%d*$", ""):gsub("^0+", "") == #compact_val:gsub("%.%d*$", "") then
					...
					compact_val = checker.raw_format_sig(
						negt,
						post,
						0,
						self.maximumSignificantDigits or csize or #compact_val:gsub("%.%d*$", ""),
						self.rounding
					)
					intlen = intlen + 1
					if self.compactPattern.other[math.min(intlen, 12)] then
						compact_val = compact(
							compact_val,
							self.compactPattern.other[math.min(intlen, 12)].size + math.max(intlen - 12, 0)
						)
					end
					if self.isSignificant then
						post = checker.raw_format_sig(
							negt,
							compact_val,
							self.minimumSignificantDigits,
							self.maximumSignificantDigits,
		...
		elseif post == "inf" then
			post = parts and { type = "infinity", value = self.symbols.infinity, source = source }
				or self.symbols.infinity
		elseif self.notation == "standard" or self.notation == "compact" then
			--[=[ Standard formatting ]=]
			--
			local intg, frac = post:match("^(%d*)%.?(%d*)$")
			local gs = (
				(self.useGrouping ~= "never" and self.useGrouping ~= "thousands")
				and (self.standardNPattern or standardPattern).metadata.integerGroupSize
			) or ((self.useGrouping == "always" or self.useGrouping == "thousands") and { 3, 3 })
			if gs and (#intg >= gs[1] + self.minimumGroupingDigits) then
				...
				if parts or (sym:match("[%%%d]")) then
					...
					if gs[1] == gs[2] then
						ret, rem =
							parts and checker.initializepart() or checker.initializestringbuilder({}), intg:reverse()
					else
						ret, rem = intg:reverse():match(("^(%s)(.+)$"):format(("%d"):rep(gs[1])))
						ret = parts
								and checker.initializepart({
									{ type = "group", value = sym, source = source },
									{
										type = "integer",
										value = checker.substitute(ret:reverse(), self.numberingSystem),
										source = source,
									},
								})
							or checker.initializestringbuilder({
								sym,
								checker.substitute(ret:reverse(), self.numberingSystem),
							})
					end
					for r in rem:gmatch("%d" .. ("%d?"):rep(gs[2] - 1)) do
						ret = checker.addtofirstpart(
							"group",
							sym,
							source,
							checker.addtofirstpart(
								"integer",
								checker.substitute(r:reverse(), self.numberingSystem),
								source,
								ret
							)
						)
				...
				elseif gs[1] == gs[2] then
					intg = checker.substitute(
						intg:reverse():gsub(("%d"):rep(gs[1]), "%1" .. sym:reverse(), (#intg - 1) / gs[1]):reverse(),
					...
					intg = checker.initializestringbuilder({
						checker.substitute(
							rem:gsub(("%d"):rep(gs[2]), "%1" .. sym:reverse(), (#rem - 1) / gs[2]):reverse(),
							self.numberingSystem
						),
						sym,
						checker.substitute(ret:reverse(), self.numberingSystem),
			...
			elseif parts then
				...
				post = intg
					.. ((self.style == "currency" and self.symbols.currencyDecimal) or self.symbols.decimal)
					.. checker.substitute(frac, self.numberingSystem)
			end
		else
			local intg, frac = post:match("^(%d*)%.?(%d*)$")
			if parts then
				post = {
					...
					frac ~= "" and {
						type = "decimal",
						value = ((self.style == "currency" and self.symbols.currencyDecimal) or self.symbols.decimal),
						source = source,
					},
					frac ~= "" and {
						type = "fraction",
						value = checker.substitute(frac, self.numberingSystem),
						source = source,
					},
					{ type = "exponentSeparator", value = self.symbols.exponential, source = source },
					{
						type = "exponentInteger",
						value = checker.substitute(expt, self.numberingSystem),
						source = source,
					},
				}
				if not post[2] then
				...
				post = checker.initializestringbuilder({
					...
					(
						frac == "" and ""
						or ((self.style == "currency" and self.symbols.currencyDecimal) or self.symbols.decimal)
					),
		...
		local standard = format_pattern(
			self,
			selectedPattern and (parts and checker.initializepart() or checker.initializestringbuilder({})) or start,
			post,
			pattern,
			negt,
			false,
		...
		if selectedPattern then
			standard = format_pattern(
				self,
				start,
				standard,
				(negt and selectedPattern.negtoken) or selectedPattern.postoken or selectedPattern,
				negt,
				true,
				source
			)
	...
	--
	local ret = ret1
			and (parts and checker.formattoparts(nil, checker.initializepart(), self.rangePattern, "shared", ret0, ret1) or self.rangePattern:gsub(
				"{[01]}",
				{ ["{0}"] = table.concat(ret0), ["{1}"] = table.concat(ret1) }
			))
	...
	local currencyName = (
		self.currencyData and checker.negotiate_plural_table(self.currencyData, self.pluralRule, rawvalue0, rawvalue1)
	)
	local unit0 = parts
			and checker.formattoparts(
				self.style == "unit" and "unit",
				checker.initializepart(),
				unitPattern,
				range and "shared" or nil,
				ret,
				currencyName and { type = "currency", value = currencyName }
			)
	...
	if self.compoundUnitPattern then
		if self.unitPattern1.perUnitPattern then
			return parts
				...
				or (self.unitPattern1.perUnitPattern:gsub("{0}", unit0 or ""))
		end
		...
		return parts
			...
			or self.compoundUnitPattern:gsub("{[01]}", { ["{0}"] = unit0 or "", ["{1}"] = unit1 or "" })
	end
