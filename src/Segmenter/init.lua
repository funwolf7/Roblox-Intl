...
local dict_len = math.max(
	dictionaries.CJ.max_length,
	dictionaries.Thai.max_length,
	dictionaries.Khmer.max_length,
	dictionaries.Lao.max_length,
	dictionaries.Burmese.max_length
)
		...
		while no_break do
			...
			if codes[i] then
				...
				elseif self.granularity == "word" then
					...
					if
						not (
							isproperty("Word_Break", codes[i], "Newline")
							or isproperty("Word_Break", codes[i], "CR")
							or isproperty("Word_Break", codes[i], "LF")
						)
						and (
							isproperty("Word_Break", codes[i + 1], "Format")
							or isproperty("Word_Break", codes[i + 1], "Extend")
						)
					then
						suppressed = true
					end
				...
				if (not no_break) and self.granularity == "word" and #ret <= dict_len then
					...
					for i1 = 1, dict_len do
						...
						if
							dictionaries.CJ.data[check]
							or dictionaries.Thai.data[check]
							or dictionaries.Lao.data[check]
							or dictionaries.Khmer.data[check]
							or dictionaries.Burmese.data[check]
						then
							highest_i = i1
					...
					if highest_i > 0 then
						...
						for i1 = 0, dict_len - 1 do
							...
							check = check .. utf8.char(codes[i + i1])
							if
								(i1 > i1_len and dictionaries.CJ.data[check])
								or (i1 == i1_len and (dictionaries.CJ.data[check] or 0) > freq)
								or (
									i1 >= i1_len
									and (
										dictionaries.Thai[check]
										or dictionaries.Lao.data[check]
										or dictionaries.Khmer.data[check]
										or dictionaries.Burmese.data[check]
									)
...
local function tokenize(id, rule)
	...
	elseif id == "WB4" then
		...
		return function(codes, i)
			return (
				isproperty("Word_Break", codes[i - 1], "Newline")
				or isproperty("Word_Break", codes[i - 1], "CR")
				or isproperty("Word_Break", codes[i - 1], "LF")
			) and (isproperty("Word_Break", codes[i], "Format") or isproperty("Word_Break", codes[i], "Extend")),
				false
		end
	elseif id == "WB6" then
		return function(codes, i)
			return isproperty("Word_Break", codes[i], "ALetter")
				and isproperty("Word_Break", codes[i + 2], "ALetter")
				and isproperty("Word_Break", codes[i + 1], "MidLetter")
				and isproperty("Word_Break", codes[i + 1], "MidNumLet"),
				false
		end
	elseif id == "WB7" then
		return function(codes, i)
			return isproperty("Word_Break", codes[i - 1], "ALetter") and isproperty(
				"Word_Break",
				codes[i + 1],
				"ALetter"
			) and isproperty("Word_Break", codes[i], "MidLetter") and isproperty("Word_Break", codes[i], "MidNumLet"),
				false
		end
	elseif id == "WB11" then
		...
		return function(codes, i)
			if
				(isproperty("Word_Break", codes[i], "MidNum") or isproperty("Word_Break", codes[i], "MidNumLetQ"))
				and (
					isproperty("Word_Break", codes[i - 1], "Numeric")
					and isproperty("Word_Break", codes[i + 1], "Numeric")
				)
			then
				return true, false
			end
	...
	elseif id == "SB7" then
		return function(codes, i)
			return isproperty("Sentence_Break", codes[i - 1], "Upper") and isproperty(
				"Sentence_Break",
				codes[i],
				"ATerm"
			) and isproperty("Sentence_Break", codes[i + 1], "Upper"),
				false
		end
	elseif id == "SB8" then
		return function(codes, i)
			if
				(
					isproperty("Sentence_Break", codes[i], "ATerm")
					or (isproperty("Sentence_Break", codes[i - 1], "ATerm") and (isproperty(
						"Sentence_Break",
						codes[i],
						"Close"
					) or isproperty("Sentence_Break", codes[i], "ATerm")))
					or (
						isproperty("Sentence_Break", codes[i - 2], "ATerm")
						and isproperty("Sentence_Break", codes[i - 1], "Close")
						and isproperty("Sentence_Break", codes[i], "Sp")
					)
				) and (isproperty("Sentence_Break", codes[i + 2], "Lower"))
			then
				return true, false
			end
			return false, false
	...
	elseif id == "SB8.1" then
		return function(codes, i)
			if
				(
					(isproperty("Sentence_Break", codes[i], "STerm") or isproperty("Sentence_Break", codes[i], "ATerm"))
					or ((isproperty("Sentence_Break", codes[i - 1], "STerm") or isproperty(
						"Sentence_Break",
						codes[i - 1],
						"ATerm"
					)) and (isproperty("Sentence_Break", codes[i], "Close") or isproperty(
						"Sentence_Break",
						codes[i],
						"Sp"
					)))
					or (
						(
							isproperty("Sentence_Break", codes[i - 1], "STerm")
							or isproperty("Sentence_Break", codes[i - 1], "ATerm")
						)
						and isproperty("Sentence_Break", codes[i - 1], "Close")
						and isproperty("Sentence_Break", codes[i], "Sp")
					)
				...
				and (
					isproperty("Sentence_Break", codes[i + 1], "SContinue")
					or isproperty("Sentence_Break", codes[i + 1], "STerm")
					or isproperty("Sentence_Break", codes[i + 1], "ATerm")
				)
			then
				return true, false
				...
				(
					...
					or (
						(
							isproperty("Sentence_Break", codes[i - 1], "STerm")
							or isproperty("Sentence_Break", codes[i - 1], "ATerm")
						) and isproperty("Sentence_Break", codes[i], "Close")
					)
				)
				and (
					isproperty("Sentence_Break", codes[i + 1], "Close")
					or isproperty("Sentence_Break", codes[i + 1], "Sp")
					or isproperty("Sentence_Break", codes[i + 1], "Sep")
					or isproperty("Sentence_Break", codes[i + 1], "CR")
					or isproperty("Sentence_Break", codes[i + 1], "LF")
				)
			then
				return true, false
			end
		...
		return function(codes, i)
			if
				(
					...
					or (
						(
							isproperty("Sentence_Break", codes[i - 2], "STerm")
							or isproperty("Sentence_Break", codes[i - 2], "ATerm")
						)
						and isproperty("Sentence_Break", codes[i - 1], "Close")
						and isproperty("Sentence_Break", codes[i], "Sp")
					)
				...
				and (
					isproperty("Sentence_Break", codes[i + 1], "Sp")
					or isproperty("Sentence_Break", codes[i + 1], "Sep")
					or isproperty("Sentence_Break", codes[i + 1], "CR")
					or isproperty("Sentence_Break", codes[i + 1], "LF")
				)
		...
		return function(codes, i)
			if
				...
				or ((isproperty("Sentence_Break", codes[i - 1], "STerm") or isproperty(
					...
					codes[i],
					"Sp"
				) or isproperty("Sentence_Break", codes[i], "Sep") or isproperty("Sentence_Break", codes[i], "CR") or isproperty(
					"Sentence_Break",
					codes[i],
					"LF"
				)))
				or ((isproperty("Sentence_Break", codes[i - 2], "STerm") or isproperty(
					"Sentence_Break",
					codes[i - 2],
					"ATerm"
				)) and isproperty("Sentence_Break", codes[i - 1], "Close") and (isproperty(
					"Sentence_Break",
					codes[i],
					"Sp"
				) or isproperty("Sentence_Break", codes[i], "Sep") or isproperty("Sentence_Break", codes[i], "CR") or isproperty(
				...
				or ((isproperty("Sentence_Break", codes[i - 2], "STerm") or isproperty(
					...
					codes[i - 2],
					"ATerm"
				)) and isproperty("Sentence_Break", codes[i - 1], "Sp") and (isproperty(
					"Sentence_Break",
					codes[i],
					"Sep"
				) or isproperty("Sentence_Break", codes[i], "CR") or isproperty("Sentence_Break", codes[i], "LF")))
				or (
					(
						isproperty("Sentence_Break", codes[i - 3], "STerm")
						or isproperty("Sentence_Break", codes[i - 3], "ATerm")
					)
					and isproperty("Sentence_Break", codes[i - 2], "Close")
					and isproperty("Sentence_Break", codes[i - 1], "Sp")
					and (
						isproperty("Sentence_Break", codes[i], "Sep")
						or isproperty("Sentence_Break", codes[i], "CR")
						or isproperty("Sentence_Break", codes[i], "LF")
					)
				)
			then
				return true, true
