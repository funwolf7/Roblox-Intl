...
local function r(name)
	local module_table = require(script.Parent:WaitForChild(name))
...
function modules.testFormat(locale)
	return ("%s\n%s\n%s    %s"):format(
		modules.DateTimeFormat
			.new(locale, { timeStyle = "short" })
			:Format({ year = 1, month = 1, day = 1, hour = 0, min = 34 }),
...
function modules.extendedTestFormat(locale)
	return ("%s\n%s\n%s      %s\n%s\n%s   %s   %s\n%s   %s\n%s      %s   %s\n%s   %s\n%s    %s\n%s   %s"):format(
		...
		modules.ListFormat.new(locale, { type = "conjunction" }):Format({
			modules.DisplayNames.new(locale, { type = "region" }):Of("CN"),
			modules.DisplayNames.new(locale, { type = "region" }):Of("JP"),
			modules.DisplayNames.new(locale, { type = "region" }):Of("KR"),
		}),

		modules.DateTimeFormat
			.new(locale, { timeStyle = "medium" })
			:Format({ year = 1, month = 1, day = 1, hour = 13, min = 34, sec = 45 }),
		modules.DateTimeFormat.new(locale, { dateStyle = "medium" }):Format({ year = 2012, month = 3, day = 4 }),

		modules.DateTimeFormat
			.new(locale, { dateStyle = "full", timeStyle = "medium" })
			:Format({ year = 1987, month = 6, day = 5, hour = 16, min = 53, sec = 32 }),
		...
		modules.NumberFormat
			.new(locale, { notation = "compact", minimumSignificantDigits = 3, maximumSignificantDigits = 3 })
			:Format(23456),
		...
		modules.NumberFormat
			.new(
				locale,
				{ style = "unit", unit = "inch", unitDisplay = "long", notation = "compact", compactDisplay = "long" }
			)
			:Format(1234)
	)
end
...
local function isbigint(v)
	...
	return str:match("^[nN][aA][nN]$") or str:match("^-?[iI][nN][fF]$") or str:match(
		"^-?[iI][nN][fF][iI][nN][iI][tT][yY]$"
	) or (str:find("[eE]") and str:match("^%d*%.?%d*[eE][-+]?%d+$")) or str:match("^-?%d+%.?%d*$"),
		str
end
...
local function is_latin(c)
	return c
		and (
			(c >= 0x0041 and c <= 0x005A)
			or (c >= 0x0061 and c <= 0x007A)
			or (c == 0x00AA)
			or (c == 0x00BA)
			or (c >= 0x00C0 and c <= 0x00D6)
			or (c >= 0x00D8 and c <= 0x00F6)
			or (c >= 0x00F8 and c <= 0x02B8)
			or (c >= 0x02E0 and c <= 0x02E4)
			or (c >= 0x1D00 and c <= 0x1D25)
			or (c >= 0x1D2C and c <= 0x1D5C)
			or (c >= 0x1D62 and c <= 0x1D65)
			or (c >= 0x1D6B and c <= 0x1D77)
			or (c >= 0x1D79 and c <= 0x1DBE)
			or (c >= 0x1E00 and c <= 0x1EFF)
			or (c == 0x2071)
			or (c == 0x207F)
			or (c >= 0x2090 and c <= 0x209C)
			or (c >= 0x212A and c <= 0x212B)
			or (c == 0x2132)
			or (c == 0x214E)
			or (c >= 0x2160 and c <= 0x2188)
			or (c >= 0x2C60 and c <= 0x2C7F)
			or (c >= 0xA722 and c <= 0xA787)
			or (c >= 0xA78B and c <= 0xA78E)
			or (c >= 0xA790 and c <= 0xA793)
			or (c >= 0xA7A0 and c <= 0xA7AA)
			or (c >= 0xA7F8 and c <= 0xA7FF)
			or (c >= 0xFB00 and c <= 0xFB06)
...
local function tolower(self)
	for i, v in ipairs(self) do
		...
		if
			self[i] == 0x03A3
			and (is_latin(self[i - 1]) or is_greek(self[i - 1]))
			and ((not self[i + 1]) or table.find(whitespaces, self[i + 1]))
		then
			self[i] = 0x03C2
		else
...
local function totitle_whitespace_index(self)
	...
	for i, v in ipairs(self) do
		if table.find(whitespaces, self[i]) then
			...
			if
				self[i] == 0x03A3
				and is_latin(self[i - 1])
				and ((not self[i + 1]) or table.find(whitespaces, self[i + 1]))
			then
				self[i] = 0x03C2
			else
				self[i] = (last_whitespace and (casing.caseMapping.title[v] or casing.caseMapping.upper[v]))
					or ((not last_whitespace) and casing.caseMapping.lower[v])
					or v
			end
	...
	if type(locales) == "table" then
		...
		for _, v in next, ipairs(locales) do
			if type(v) == "string" or private.Locale.intl_proxy[v] then
				table.insert(
					ret,
					tostring(private.Locale.intl_proxy[locales] and locales or modules.Locale.new(locales))
				)
			elseif v ~= nil then
