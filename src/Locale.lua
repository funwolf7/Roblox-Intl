...
local check = {
	language = function(str)
		return str
			and (#str ~= 4 and str:match("^%a%a%a?%a?%a?%a?%a?%a?$"))
			and localedata.getalias("languageAlias", str:lower())
	end,
	script = function(str, nomod)
		return str
			and str:match("^%a%a%a%a$")
			and (nomod and str or localedata.getalias("scriptAlias", title_case(str)))
	end,
	region = function(str, nomod)
		return str
			and (str:match("^%a%a$") or str:match("^%d%d%d$"))
			and (nomod and str or localedata.getalias("territoryAlias", str:upper()))
	end,
	variants = function(str, nomod)
		return str
			and (str:match("^%d%w%w%w$") or str:match("^%w%w%w%w%w%w?%w?%w?$"))
			and (nomod and str or str:lower())
	end,
	u = function(str)
...
function methods:GetVariants()
	return table.move(
		intl_proxy[self].variants,
		1,
		#intl_proxy[self].variants,
		1,
		table.create(#intl_proxy[self].variants)
	)
end
...
local function tostr(self)
	...
	return self.language
		...
		.. (self.variants[1] and (#self.variants > 1 or self.variants[1] ~= "posix") and ("-" .. table.concat(
			self.variants,
			"-"
		)) or "")
		.. (u_ext or "")
...
function l.new(...)
	...
	if type(value) == "string" then
		...
		data.baseName = data.language
			...
			.. (
				data.variants[1]
					and (#data.variants == 1 and data.variants[1] == "posix" and "-u-va-posix" or ("-" .. table.concat(
						data.variants,
						"-"
					)))
				or ""
