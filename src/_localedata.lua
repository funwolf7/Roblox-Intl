...
function d.rawmaximize(locale, exclude_und)
	...
	local ret_language, ret_script, ret_region =
		d.getlocaleparts(ret0 or ret1 or ret2 or supplemental.likelySubtags[language] or locale)
...
function d.negotiateparent(locale)
	...
	return parentlocale[locale]
		or (locale:match("%-") and locale:gsub("%-%w+$", "") or (locale ~= "root" and "root" or nil))
end
...
local function rawgetdata(ttype, locale)
	...
	return deepcopymerge(
		rawgetdata(ttype, d.negotiateparent(locale)),
		requireifnotnil(commons[ttype]:FindFirstChild(locale))
	)
end
...
function d.getdata(ttype, locale)
	...
	local ms = resolve_alias(
		deepcopymerge(
			rawgetdata(ttype, locale)
				or rawgetdata(ttype, d.negotiateparent(minimized))
				or rawgetdata(ttype, d.negotiateparent(maximized)),
			requireifnotnil(commons[ttype]:FindFirstChild(locale))
				or requireifnotnil(commons[ttype]:FindFirstChild(minimized))
				or requireifnotnil(commons[ttype]:FindFirstChild(maximized))
		)
	)
...
function d.exists(ttype, locale)
	...
	local minimized, maximized = d.minimizestr(locale, true), d.maximizestr(locale, true)
	return not not (
		d.exists(d.negotiateparent(minimized))
		or d.exists(d.negotiateparent(maximized))
		or commons[ttype]:FindFirstChild(locale)
		or commons[ttype]:FindFirstChild(minimized)
		or commons[ttype]:FindFirstChild(maximized)
	)
end
