...
local function negotiate_table(tbl, index, style)
	...
	return (style == "narrow" and tbl[index .. "-narrow"])
		or ((style == "short" or style == "narrow") and tbl[index .. "-short"])
		or tbl[index]
end
...
local function parselangugage(displaynames, pattern, code, style, fallback)
	...
	if
		script
		and region
		and negotiate_table(displaynames.languages, language .. "-" .. script .. "-" .. region, style)
	then
