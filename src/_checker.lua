...
local function check_property(tbl_out, tbl_to_check, property, default)
	...
	elseif type(value) == "number" and (value % 1 == 0) or (value == math.huge) then
		...
		valid = (value >= (tbl_out[min] or tonumber(min) or 0))
			and ((max == "" and value ~= math.huge) or (value <= tonumber(max)))
	end
	if valid then
...
function c.supportedlocale(ttype, locales)
	...
	elseif type(locales) == "string" or Locale._private.intl_proxy[locales] then
		return {
			localedata.exists("main", Locale._private.intl_proxy[locales] and locales or Locale.new(locales))
					and tostring(locales)
				or nil,
		}
	elseif locales == nil then
		return {}
...
function c.options(ttype, locales, options)
	...
	if t == "nu" or t == "dt" or t == "rt" then
		check_property(
			...
			options,
			"g/numberingSystem",
			valid_value_property["g/numberingSystem"][table.find(
				valid_value_property["g/numberingSystem"],
				locale.numberingSystem
			)] or data.numbers.defaultNumberingSystem
		)
		if t ~= "dt" then
			...
			check_property(
				...
				options,
				"nu/useGrouping",
				(ret.notation == "compact") and "min2" or "auto"
			)
	...
	elseif t == "nu" then
		...
		ret.minimumGroupingDigits = ((ret.useGrouping == "always" or ret.useGrouping == "thousands") and 1)
			or math.max(ret.useGrouping == "min2" and 2 or 1, numbers.minimumGroupingDigits)

		local decimalPattern = c.negotiate_numbering_system(
			numbers.formats[(ret.style == "percent" and ret.notation ~= "compact") and "percent" or "decimal"],
			ret.numberingSystem,
		...
		if ret.style == "currency" and (ret.currencyDisplay ~= "name") then
			...
			local currencyFormat = c.negotiate_numbering_system(
				numbers.formats.currency,
				ret.numberingSystem,
				numbers.defaultNumberingSystem
			)
			ret[ret.notation == "compact" and "standardNPattern" or "standardPattern"] = currencyFormat[ret.currencySign]
				or currencyFormat.standard
		...
		if ret.style == "unit" or (ret.style == "currency" and ret.currencyDisplay == "name") then
			if ret.style == "unit" then
				...
				ret.standardNPattern = c.negotiate_numbering_system(
					numbers.formats.currency,
					ret.numberingSystem,
					numbers.defaultNumberingSystem
				).standard
				ret.unitPattern = c.negotiate_numbering_system_index(
					"unitPattern",
					numbers.formats.currency,
					ret.numberingSystem,
					numbers.defaultNumberingSystem
		...
		if ret.notation == "compact" then
			if ret.style == "currency" and (ret.currencyDisplay ~= "name") then
				ret.compactPattern = c.negotiate_numbering_system_index(
					"short",
					numbers.formats.currency,
					ret.numberingSystem,
					numbers.defaultNumberingSystem
		...
		ret.rangePattern = c.negotiate_numbering_system_index(
			"range",
			numbers.misc,
			ret.numberingSystem,
			numbers.defaultNumberingSystem
	...
	elseif t == "dt" then
		...
		check_property(
			...
			options,
			"g/calendar",
			valid_value_property["g/calendar"][table.find(valid_value_property["g/calendar"], locale.calendar)]
				or (calpref[region] or calpref["001"]):gsub(" .*$", "")
		)
		...
		check_property(
			...
			options,
			"g/hourCycle",
			valid_value_property["g/hourCycle"][table.find(valid_value_property["g/hourCycle"], locale.hourCycle)]
		)
	...
	elseif t == "lf" then
		...
		local p =
			data.listPatterns[(ret.type:gsub("disjunction", "or"):gsub("conjunction", "standard")) .. (ret.style == "long" and "" or ("-" .. ret.style))]
		ret.t, ret.s, ret.m, ret.e =
			c.tokenizeformat(p["2"]),
			c.tokenizeformat(p["start"]),
			c.tokenizeformat(p["middle"]),
			c.tokenizeformat(p["end"])
	elseif t == "sg" then
		...
		check_property(
			...
			options,
			"g/suppression",
			valid_value_property["g/suppression"][table.find(valid_value_property["g/suppression"], locale.suppression) or 1]
		)
	...
	if t == "nu" or t == "pr" or t == "rt" then
		...
		ret.isSignificant = not not (
			rawget(options, "minimumSignificantDigits") or rawget(options, "maximumSignificantDigits")
		)
		if ret.isSignificant then
			...
			if not (ret.minimumFractionDigits or ret.maximumFractionDigits) then
				if ret.style == "currency" and ret.notation ~= "compact" then
					...
					local digits = (currencyDataFractions[ret.currency] and currencyDataFractions[ret.currency]._digits)
						or 2
...
function c.addpart(self, ttype, value, source)
	return ((getmetatable(self) == string_builder_mt or type(self) == "string") and self or c.initializepart(self))
		.. (
			(getmetatable(self) == string_builder_mt or type(self) == "string") and value
			or { type = ttype, value = value, source = source }
		)
end
function c.addtofirstpart(ttype, value, source, self)
	return (
		(getmetatable(self) == string_builder_mt or type(self) == "string") and value
		or { type = ttype, value = value, source = source }
	) .. ((getmetatable(self) == string_builder_mt or type(self) == "string") and self or c.initializepart(self))
end
...
function c.negotiate_plural_table(tbl, plural_rule, value0, value1)
	return (not value1 and (tbl[tostring(value0)] or tbl[tostring(value0)]))
		or tbl[value1 and plural_rule:SelectRange(value0, value1) or plural_rule:Select(value0)]
		or tbl["other"]
end
...
local function quantize(isneg, val, exp, rounding)
	...
	elseif rounding:match("^half") then
		add = d[pos]:match(
			((rounding == "halfEven" and (d[pos - 1] or "0"):match("[02468]")) or rounding == "halfDown") and "[6-9]"
				or "[5-9]"
		)
	end
	for p = pos, #d do
...
function c.raw_format_sig(negval, val, min, max, rounding)
	...
	if max and max ~= math.huge then
		...
		intg, frac = quantize(
			negval == "-",
			val,
			max - ((val:find("%.") or (#val + 1)) - 1) + #val:gsub("%.", ""):match("^0*"),
			rounding
		)
		intg, frac = intg:gsub("^0+", ""), frac:gsub("0+$", "")
		intg = intg == "" and "0" or intg
